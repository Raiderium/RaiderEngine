 ...
 * Physical body.
 * 
 * A body must outlive any joints and shapes attached to it.
 * Note that for all properties and functions, 'local' means relative to the body transform.
 */
final class Body
{package:
	World _world;
...
public:
	...
	{
		_world = world;
		dbody = dBodyCreate(_world.dworld);
	}
	...
	@property void massSphere(double radius)
	{
		dMass m; dBodyGetMass(dbody, &m);
	...
	@property void massBox(vec3 dim)
	{
		dMass m; dBodyGetMass(dbody, &m);
